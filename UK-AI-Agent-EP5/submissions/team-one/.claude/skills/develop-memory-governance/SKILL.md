...
- `test_bouncer.py` — the test suite (currently ~29 tests). Every fix gets a regression test here.
...
M1 bouncer ✅ · M2 tests ✅ · audit log + tamper-evidence ✅ ·
**Next:** M3 stub users in `users.yaml` (roles→allowed categories; CEO = `exec` with `deny:[legal]`),
`memory.py` loaders · M5 lineage-based revocation (DERIVED_FROM graph; revoke a source → propagate
to all derivatives; revoked beats allowed) · M6 one `cli.py` entrypoint ·
...
- Python interpreter (call by full path; PATH has a fake stub): 
...
- A virtual environment ("toolbox") lives in `team-one/.venv` (git-ignored). It holds `pytest`.
- Throwaway/probe scripts go in the scratchpad temp dir, NOT the project folder; use a TEMP log
