...
```
User
  → GOVhence (LLM)                          receive message + simple profile
...
| **GOVhence** | LLM — **sole orchestrator** | Receives the message, reads the profile, routes work to every other component, waits where needed, returns the answer. Nothing bypasses it. |
...
2. **GOVhence reads the Profile Store** for the user's profile + access tags (role, department, RBAC/ACL).
   These identity/access tags are **never** produced or guessed by an LLM.
...
5. **If Read = yes → GOVhence → Bouncer** with the strict tags: the content tags **plus** the user's
   access tags (read from the Profile Store in step 2). The **Bouncer is script-only (no LLM)** — strict
   tag match + RBAC/ACL + access inheritance — and returns the permitted matching memories = **MemoryLane**.
   **GOVhence waits** for the Bouncer.
...
sequenceDiagram
    ...
    participant GOV as GOVhence (orchestrator)
    ...
    alt Read = yes
        GOV->>Bouncer: content tags + access tags (from Profile Store)
...
flowchart TD
    ...
    G["GOVhence<br/>sole orchestrator"]
    C["Classifier LLM"]
    ...
    G -- "if Read: content + access tags" --> B
    B -- "MemoryLane" --> G
...
| **Bouncer** (no LLM) | content tags + user access tags | Permitted matching memories = **MemoryLane** (strict, fail-closed). |
...
- **Profile/access tags are never LLM-derived.** Role, department, and RBAC/ACL tags **always** come
  directly from the User Profile Store. No LLM (Classifier, Judge, Memoriser, Responder, or GOVhence)
  invents, infers, or guesses them. *(This is the dotted line in the diagram — an identity-integrity /
